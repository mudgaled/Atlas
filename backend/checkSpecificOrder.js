...
const checkSpecificOrder = async () => {
  try {
    ...
    if (order) {
      ...
      console.log('Order User:', order.user ? `${order.user.name} (${order.user._id}, ${order.user.role})` : 'N/A');
      console.log('Order Seller:', order.seller ? `${order.seller.name} (${order.seller._id}, ${order.seller.role})` : 'N/A');
